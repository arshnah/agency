    ...
    <section id="services" className="relative py-24 md:py-32 px-6 md:px-14 bg-black overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div ref={gridRef} className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {SERVICES.map((s) => (
            <div key={s.n} className="svc-card relative p-8 rounded-2xl overflow-hidden group before:content-[''] before:absolute before:top-0 before:-left-[120%] before:w-[55%] before:h-full before:bg-[linear-gradient(105deg,transparent_30%,rgba(255,255,255,0.08)_50%,transparent_70%)] before:animate-[shimmer_7s_ease-in-out_infinite] before:pointer-events-none"
              style={{
              ...
              <span className="absolute bottom-7 right-7 text-acid/30 group-hover:text-acid group-hover:translate-x-1 group-hover:-translate-y-1 transition-all text-sm">↗</span>
            </div>
          ))}
        </div>
      </div>
      <style>{`@keyframes shimmer { 0%,100% { left: -120%; } 60% { left: 160%; } }`}</style>
    </section>
  )
}
